...
router.get('/v3/covid19/historical', async (req, res) => {
...
router.get('/v3/covid19/historical/usacounties', async (req, res) => {
	const allData = scraper.historical.getHistoricalUSAProvincesV2(JSON.parse(await redis.get(keys.historical_v2_USA)));
	res.send(allData);
});

router.get('/v3/covid19/historical/usacounties/:state', async (req, res) => {
...
router.get('/v3/covid19/historical/all', async (req, res) => {
	const { lastdays } = req.query;
	res.send(await scraper.historical.getHistoricalAllDataV2(JSON.parse(await redis.get(keys.historical_v2)), lastdays));
});

router.get('/v3/covid19/historical/:query/:province?', async (req, res) => {
