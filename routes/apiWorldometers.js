...
router.get('/v2/all', async (req, res) => {
	const { yesterday, yesterday2, allowNull } = req.query;
	const data = await getAllData(wordToBoolean(yesterday) ? keys.yesterday_countries : wordToBoolean(yesterday2) ? keys.yesterday2_countries : keys.countries);
	res.send(!wordToBoolean(allowNull) ? countryUtils.transformNull(data) : data);
});

router.get('/v2/countries', async (req, res) => {
	const { sort, yesterday, yesterday2, allowNull } = req.query;
	const countries = JSON.parse(await redis.get(wordToBoolean(yesterday) ? keys.yesterday_countries : wordToBoolean(yesterday2) ? keys.yesterday2_countries : keys.countries))
...
router.get('/v2/countries/:query', async (req, res) => {
	const { yesterday, yesterday2, strict, allowNull } = req.query;
	const { query } = req.params;
	let countries = JSON.parse(await redis.get(wordToBoolean(yesterday) ? keys.yesterday_countries : wordToBoolean(yesterday2) ? keys.yesterday2_countries : keys.countries))
